...
The requirements published on the Steam page are:

| Requirement | Minimum | Recommended |
|-------------|---------|-------------|
| **OS** | Windows 10 64-bit | Windows 10 64-bit |
| **CPU** | Quad Core 2.4 GHz | Quad Core 2.4 GHz |
| **RAM** | 8 GB | 16 GB |
...
- **The CPU requirement is identical on both tiers** — a modern quad-core at 2.4 GHz+ is the baseline; the upgrade path is mostly GPU and RAM.
- **An SSD is only recommended, not required** — the game will run from a hard drive, but load times between regions and runs improve noticeably on an SSD.
- **Windows 10 64-bit minimum** — if you're on Windows 11, you're fine; no macOS or Linux versions have been announced.

## What the Specs Mean for Spell Effects Performance

Echoes of Mystralia is a **particle-heavy game by design**. Spellcrafting means dozens of simultaneous effects — chain lightning, freezing tornadoes, fire vortices, and summoned projectiles — all stacking on screen at once, especially against elite packs and Guardian bosses.

That has direct hardware implications:
...
**Steam Deck compatibility has not been officially confirmed (TBD).** Borealys Games has not published a Steam Deck verification status or a playability note.

What we know:

- The game is **controller-supported on Steam** — the demo supports both keyboard/mouse and gamepad.
- The **free demo runs on Steam now**, so you can test Steam Deck compatibility today by installing the demo in your library and running it in Desktop Game Mode or via Proton.
- The minimum specs (quad-core CPU, 8 GB RAM) are within Steam Deck hardware territory, but GPU behavior under full particle stacks on the Deck's APU is untested territory — the demo is the only honest way to check.

## Graphics Settings Overview

Borealys has **not published a full settings list for Early Access (TBD)**. Based on the demo, you can expect the standard modern PC suite:

- **Resolution & fullscreen modes** — including borderless windowed
- **Quality presets** — low/medium/high-style presets that bundle texture, shadow, and effect quality
- **V-Sync and frame limiting**
- **Effect/particle quality** — the single most important setting for this game; lowering it is the fastest way to stabilize frame rate during particle-heavy fights

Anything more specific is **TBD** until Borealys documents the EA build. For the demo, the same GPU-bound rule applies: if you see dips during spell-heavy encounters, drop the effect-quality setting before touching resolution.
...
3. **Compare at both minimum and your actual settings** to find your comfortable preset
4. **Test your controller and Steam Deck setup** while you're at it

And remember — **your demo save transfers to Early Access**, so testing isn't wasted progress. See the [EA Launch Guide](/guides/ea-launch-guide) for the full first-session checklist and the [Demo Save Transfer Guide](/guides/demo-save-transfer) for exactly what carries over.

## Related Guides

- [EA Launch Guide](/guides/ea-launch-guide) — the August 11 first-session checklist
- [FAQ](/guides/faq) — quick answers on platforms, price, and release date
- [Demo Save Transfer Guide](/guides/demo-save-transfer) — does your demo progress carry to Early Access?
