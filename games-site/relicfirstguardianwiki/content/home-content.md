---
title: "The Relic: First Guardian Wiki — Complete Guide Hub"
description: "The ultimate community wiki for The Relic: First Guardian — the Korean dark-fantasy soulslike action RPG by Project Cloud Games / Perp Games. Master 5 weapon families, 70+ relic fragments, the 70-80 Brutal bosses, unique artifacts, and more."
category: "Home"
version: "1.0"
updated: "2026-08-11"
keywords: ["The Relic: First Guardian", "The Relic: First Guardian wiki", "The Relic First Guardian guide", "Project Cloud Games", "soulslike action RPG", "Korean dark fantasy"]
related: ["beginner-guide", "weapons-guide", "builds-guide", "boss-strategies", "runes-guide"]
---

# The Relic: First Guardian Wiki — Complete Guide Hub

**The ultimate resource for mastering The Relic: First Guardian — launched July 31, 2026 on Steam and PS5. The game is out now.**

Set in the ruined world of Arsilthus, The Relic: First Guardian is a Korean dark-fantasy soulslike action RPG developed by Project Cloud Games and published by Perp Games. As the last Guardian, you must recover shattered relic fragments, master 5 weapon families with 12 skill trees (two per family), collect 70+ relic fragments, and defeat 70-80 "Brutal" bosses to seal the encroaching Void.

---

## Quick Navigation

| Guide | Best For | Difficulty |
|-------|----------|------------|
| [Beginner Guide](/guides/beginner-guide) | First-time players | Starter |
| [Weapons Guide](/guides/weapons-guide) | Choosing your weapon | Starter |
| [Best Builds Guide](/guides/builds-guide) | Build optimization | Intermediate |
| [Boss Strategies](/guides/boss-strategies) | Boss strategies | Intermediate |
| [Runes & Memory Fragments](/guides/runes-guide) | Rune collection | Intermediate |
| [Artifacts Guide](/guides/artifacts-guide) | Equipment system | Intermediate |
| [Combat Guide](/guides/combat-guide) | Mechanics deep dive | Starter |
| [Tier List](/guides/tier-list) | Weapon rankings | Starter |
| [Walkthrough](/guides/walkthrough) | Story progression | Intermediate |
| [Controls & Settings](/guides/controls-settings-guide) | PC/PS5 setup | Starter |
| [All Bosses List](/guides/all-bosses-list) | Every one of the 70-80 Brutals | Intermediate |
| [Tips & Tricks](/guides/tips-and-tricks) | 20 essential pro tips | Starter |
| [Trophy Guide](/guides/trophy-guide) | PS5/Steam achievements | Intermediate |
| [Co-Op Guide](/guides/co-op-guide) | Play with friends | Starter |
| [Release Date & Platforms](/guides/release-date-platforms) | Editions, price, launch info | All |
| [Difficulty Guide](/guides/difficulty-guide) | Easy mode explained | Starter |
| [Endings Guide](/guides/endings-guide) | All endings & unlock conditions | Advanced |
| [Classes & Archetypes](/guides/classes-guide) | Choosing your playstyle | Starter |
| [FAQ](/faq) | Quick answers | All |

---

## Featured Guides

### Weapons Guide
Master all 5 weapon families — Sword & Shield, Longsword, Dagger, Two-Handed, and Battle Staff — with 12 skill trees total (two per family). [Read the full guide →](/guides/weapons-guide)

### Boss Strategies
Strategies for defeating the 70-80 "Brutal" bosses. Learn attack patterns, the parry-gate vs dodge-gate split, and the tragic lore behind each encounter. [Browse bosses →](/guides/boss-strategies)

### Best Builds Guide
Top-tier builds for every weapon family. Combine runes, artifacts, and skill trees for maximum damage. [View builds →](/guides/builds-guide)

### Runes & Memory Fragments
All 70+ memory fragment effects explained — passive bonuses that reshape your skills and combat style. [Full rune catalog →](/guides/runes-guide)

---

## What is The Relic: First Guardian?

The Relic: First Guardian is a hardcore soulslike action RPG developed by South Korean studio Project Cloud Games and published by Perp Games. It launched on **July 31, 2026** for **PC (Steam)** and **PlayStation 5**, with Xbox Series X|S and Nintendo Switch 2 versions planned for later in summer 2026.

The game is set in **Arsilthus**, a once-prosperous land reduced to wasteland after an ancient relic shattered and unleashed the creeping Void blight. You play as the **last Guardian**, the world's last hope, tasked with recovering scattered relic fragments and sealing the void before all is consumed.

The game blends **dark fantasy with Korean folklore**, featuring quests inspired by traditional Korean tales told through environmental storytelling.

---

## Key Systems at a Glance

- **5 Weapon Families** — Sword & Shield, Longsword, Dagger, Two-Handed, Battle Staff — with 12 skill trees total (two per family)
- **No Traditional Levels** — Character growth driven by collecting 70+ relic fragments (passive effects) and unique gear
- **Stamina-Only Defense** — Attacks cost zero stamina; stamina is reserved for dodging and blocking
- **70-80 Brutal Bosses** — Each with a tragic backstory, former humans transformed by the Void
- **Artifact System** — Every weapon and armor piece is unique (no duplicate drops); artifact categories are unconfirmed
- **Semi-Open World** — Exploration rewards hidden weapons, artifact slots, and side quests
- **Skill Cooldowns** — Skills use cooldown timers instead of resource pools

---

## Quick Start Links

- [The Relic: First Guardian on Steam](https://store.steampowered.com/app/2827820/The_Relic_First_Guardian/)
- [Project Cloud Games official](https://www.projectcloudgames.com)
- [Perp Games publisher](https://perpgames.com)

---

*This wiki is community-maintained and reflects the launch version. Content will be updated as patches and updates are released.*
