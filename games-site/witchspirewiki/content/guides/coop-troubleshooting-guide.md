...
related: ["coop-multiplayer-guide", "beginner-guide", "patch-notes-updates"]
---
...
**Version rule:** everyone must be on the **same game version and branch** (including beta branches). If the invite fails or the friend never loads, check versions first — mismatched versions are the most common "can't join" cause.
...
| **Same version/branch** | Mismatched versions block joining entirely (reported) |
...
Desync (enemies/actions desyncing between players, rubber-banding in co-op) is a **known issue** — community reports describe desync fixes on the developer's QoL list. Until it ships:
...
Witchspire is currently PC-first; co-op is via the Steam friends list. Console support is part of the EA roadmap but not live yet.
...
Not fully — community reports describe it as on the developer's QoL list. Watch the [Patch Notes & Roadmap](/guides/patch-notes-updates) page for updates.
