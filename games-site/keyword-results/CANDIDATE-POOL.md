...
| 34 | Dressmaker | 缝纫模拟 (**开发 Cozy Lives / 发行 Free Lives**) | 19 | 9/21 | ❌ 无 | 8/10 | ⭐ **9/11 门禁 GO（17/20）→ 建站中** — 200K+ wishlist、itch 原型 100K+ 下载；⚠️ 归属易错：Cozy Lives 是**开发方**（Free Lives 的 cozy 子厂牌），Free Lives 是**发行方**，站级元数据 MUST NOT 写成「developed by Free Lives」；⚠️ 裸名 `Dressmaker` 是通用英文词 + Steam 另有 DressMaker(1487080)/DressMaker Pro(1599300) 两款无关应用 → SEO MUST 走长尾（`Free Lives Dressmaker` / 机制词） |
