---
title: "Taival Wiki — Complete Guide Hub"
description: "The ultimate community wiki for Taival — the 1-4 player co-op adventure RPG by Bonobo Software. Find Henki companion guides, crafting tips, biome exploration, boss strategies, and co-op tactics."
category: "Home"
version: "1.0"
updated: "2026-08-01"
keywords: ["Taival", "Taival wiki", "Taival guide", "Bonobo Software", "co-op adventure RPG", "Henki companion", "voxel open world"]
related: ["beginner-guide", "henki-companion-guide", "coop-multiplayer-guide", "crafting-professions-guide"]
---

# Taival Wiki — Complete Guide Hub

**Everything you need to explore the hand-crafted voxel world of Taival with your shapeshifting Henki companion.**

Taival launched in Steam Early Access on July 14, 2026 by Bonobo Software (Finland) and Level Up Gaming. This wiki covers all core systems, strategies, and secrets for the Early Access build. Whether you are a solo adventurer or running a 4-player co-op party, you will find practical, up-to-date information here.

---

## Quick Navigation

| Guide | Best For | Difficulty |
|-------|----------|------------|
| [Beginner Guide](/guides/beginner-guide) | First-time players | Starter |
| [Henki Companion Guide](/guides/henki-companion-guide) | Understanding your companion | Starter |
| [Best Henki Forms Tier List](/guides/best-henki-forms) | Form selection | Starter |
| [Co-op Multiplayer Guide](/guides/coop-multiplayer-guide) | Team play | Starter |
| [Crafting & Professions Guide](/guides/crafting-professions-guide) | Crafting system | Intermediate |
| [Combat System Guide](/guides/combat-system-guide) | Battle tactics | Intermediate |
| [Biomes & Exploration Guide](/guides/biomes-exploration-guide) | World navigation | Intermediate |
| [Skill Trees Guide](/guides/skill-trees-guide) | Progression planning | Intermediate |
| [Quests Walkthrough](/guides/quests-walkthrough) | Story & side quests | Intermediate |
| [Endgame & Companion Dungeons Guide](/guides/endgame-dungeons) | Endgame & solo companion challenges | Advanced |
| [Housing Guide](/guides/housing-base-building) | Base building | Intermediate |
| [FAQ](/faq) | Quick answers | All |

---

## Featured Guides

### Henki Companion Guide
Master your shapeshifting companion — learn how to tame creatures, unlock all forms, understand traits, and find rare variants. [Read the full guide →](/guides/henki-companion-guide)

### Co-op Multiplayer Guide
Online, local split-screen, and mixed mode. Group voting system, party composition strategies, and coordinated Henki tactics. [View co-op guide →](/guides/coop-multiplayer-guide)

### Best Henki Forms Tier List
Every form ranked S to C for combat, traversal, and utility. Find the optimal Henki for every situation. [Browse tier list →](/guides/best-henki-forms)

### Biomes & Exploration
All 4 biomes — highlands, forests, mountains, and ruins. Discover hidden secrets, resource hotspots, and the best Henki forms for each region. [Explore the world →](/guides/biomes-exploration-guide)

---

## What is Taival?

Taival ("journey" in Finnish) is a 1-4 player cooperative adventure RPG set in a hand-crafted voxel-style open world. Developed by the Finnish indie studio Bonobo Software and published by Level Up Gaming, it launched in Steam Early Access on July 14, 2026 at ¥77 (~US$16, with a 20% launch discount).

The game's defining feature is your **Henki** — a shapeshifting companion with no fixed form that can absorb creature essences to transform into combat beasts, swift mounts, stealth predators, gliders, and more. You and your Henki grow in parallel through dual progression systems, unlocking new abilities and forms as you explore the world.

---

## Key Systems at a Glance

- **Henki Companion** — Shapeshifting ally with combat, mount, and utility forms. Rare Golden and Purple Clone variants
- **4 Biomes** — Highlands, forests, mountains, and ancient ruins in a seamless open world
- **4-Player Co-op** — Online, local split-screen, or mixed mode with group story voting
- **Crafting & Professions** — Gather materials, unlock recipes, and craft gear at stations
- **Skill Trees** — Dual progression: character skills + Henki form mastery
- **Henki Dungeons** — Solo companion challenges with unique puzzles and exclusive rewards
- **Player Housing** — Place and upgrade homes across the world with storage and crafting stations

---

## Quick Start Links

- [Taival on Steam](https://store.steampowered.com/app/3152750/Taival/)
- [Bonobo Software](https://www.bonobosoftware.com)

---

*This wiki is community-maintained and reflects the Early Access launch build (July 2026). Content will be updated as patches and new biomes are released during the 6-12 month Early Access period.*
