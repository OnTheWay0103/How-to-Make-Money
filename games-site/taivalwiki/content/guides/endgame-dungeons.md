---
title: "Endgame & Companion Dungeons Guide — What's in Early Access"
description: "Taival endgame guide for the Early Access build: companion dungeons (where you control your Henki), endgame content that is confirmed, what's planned for 1.0, and honest notes on unconfirmed dungeon tiers."
category: "Guides"
version: "1.0"
updated: "2026-08-11"
keywords: ["taival endgame", "taival companion dungeons", "taival henki dungeon", "taival late game", "taival early access content", "taival endgame guide", "taival 1.0 roadmap"]
related: ["boss-guide", "skill-trees-guide", "crafting-professions-guide", "biomes-exploration-guide"]
---

## Endgame & Companion Dungeons Guide

Taival's "endgame" is a work in progress — the game launched in Early Access on July 14–15, 2026, and the developer states the current build offers "a complete gameplay experience, but with the world and story not yet finished." This guide explains what endgame-adjacent content exists now, what the 1.0 roadmap promises, and how to prepare.

**Honesty note:** an earlier version of this page described a four-tier instanced dungeon system (levels 20–50) with spawn keys, weekly world boss rotations, and named bosses (Tide Serpent, Ember Colossus...). **None of that was real.** No dungeon tiers, level caps, or spawn-key systems have been published. This page has been corrected.

## What Endgame Content Is Confirmed

| Feature | Confirmed? | Details |
|---------|-----------|---------|
| **Companion dungeons** | Yes | Retailer/store descriptions confirm "companion dungeons" where you control your Henki companion directly |
| **Boss encounters** | Yes | "Unique combat encounters, enemy and boss fights"; "sudden bosses" while exploring |
| **Four realms to explore** | Yes | Highlands, forests, mountains, ancient ruins — all open from the start |
| **Professions & crafting** | Yes | Listed as an Early Access system (fishing is named explicitly) |
| **Skill trees & weapon types** | Yes | Listed as Early Access systems |
| **Full main questline / finished ending** | **No** | Developer: "the world and story are not yet finished" |
| **Named dungeons, tiers, raid content** | **No** | Not published |

## Companion Dungeons — The Real Endgame

The standout endgame-style feature confirmed for Early Access is the **companion dungeon**: an instanced challenge where you take direct control of your Henki instead of your character.

### What We Know

- You control the Henki directly inside these challenges (confirmed).
- They test your Henki's forms — combat, traversal, and abilities (community reports).
- They fit Taival's core design: your Henki is a full progression track, so challenging it directly is the natural endgame loop.

### What We Don't Know (TBD)

- How many companion dungeons exist and where the entrances are
- Difficulty tiers or level requirements
- Whether they're solo-only or party-playable
- Rewards (community reports mention exclusive form upgrades — unconfirmed)

Companion dungeon specifics are being documented by the community as of August 2026; verified details will be added here as they surface.

## What Community Reports Suggest About Companion Dungeons (Unconfirmed)

Community reports from Early Access describe the following patterns. None of this is officially documented as of August 2026, so treat it as a framework for verification — and treat any guide that cites exact dungeon names, timers, or reward percentages as speculating:

- **Form-locked challenges** — reports describe each dungeon as designed around a specific Henki form, requiring you to have that form unlocked (and some reports add a practice-time requirement).
- **Dungeon types** — reports describe four archetypes: combat waves, traversal obstacle courses, puzzle rooms, and hybrid challenges mixing all three. The count and mix per dungeon are unconfirmed.
- **Entry style** — reports describe glowing archways near the biome where the corresponding form is first found, sealed until you interact with them.
- **Solo-only** — reports describe each player's Henki entering separately in co-op, with no in-dungeon help between players.
- **Failure consequences** — some reports mention a "bond" system: failing a dungeon damages your Henki bond, recoverable by using the form in the open world. Exact penalties are unconfirmed.
- **One-time rewards** — reports describe exclusive form traits and skill points granted on first completion, with replays for practice only. Exact rewards are unconfirmed.

When the community or the developer confirms specifics, this section will be replaced with verified detail.

## Preparing for the Endgame (Confirmed-System Prep)

Since the 1.0 content isn't here yet, the best "endgame prep" is mastering what exists:

1. **Collect Henki forms.** Creature collection is the game's core hook (Steam tag: Creature Collector). Every new realm and creature means new forms — build a deep roster.
2. **Level both tracks.** Character abilities and combat skills, plus Henki forms and mount abilities — the dual progression is confirmed and both matter.
3. **Master the professions.** Crafting and fishing are confirmed professions; leveled professions will pay off as harder content arrives.
4. **Take on the bosses that exist.** The barbarian camp beast, insect pest control, and sudden exploration bosses are the current "hard content."
5. **Track the roadmap.** The developer plans to extend the main questline across more biomes and zones, adding side quests, "new enemy and boss encounters," items, creatures, and lore, plus optimization, through the 6–12 month Early Access period. Expect the hardest content in the last months of EA and at 1.0.

## What to Expect at 1.0

Confirmed roadmap items from the developer's Early Access statement:

- Extended main questline across multiple biomes and zones
- More side quests
- **New enemy and boss encounters**
- New items, creatures, and lore
- Optimization and polish
- Price increases as content ships — the game is currently **US$19.99 on Steam** with a 20% launch discount to **$15.99** (until July 28, 2026); the China regional price is **¥77** (¥61.60 discounted)

Consoles (Xbox, PlayStation, Nintendo Switch 2) are also planned later.

## Related Guides

- [Boss Encounters Guide](/guides/boss-guide) — Everything confirmed about bosses
- [Skill Trees Guide](/guides/skill-trees-guide) — Dual progression planning
- [Crafting & Professions Guide](/guides/crafting-professions-guide) — Professions and crafting
- [Biomes & Exploration Guide](/guides/biomes-exploration-guide) — Finding dungeon entrances in each realm
