...
export default function Header() {
  ...
  return (
    <header className="bg-[#0a0a0a] border-b border-red-900/30 sticky top-0 z-50">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm text-gray-300 hover:text-red-400 transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="px-4 py-3 space-y-2">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-3 py-2 rounded-md text-base text-gray-300 hover:text-red-400 hover:bg-gray-800 transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
