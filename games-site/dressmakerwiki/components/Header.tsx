...
const navLinks = [
  ...
  { href: '/tier-list', label: 'Priorities' },
  { href: '/faq', label: 'FAQ' },
];
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2 shrink-0">
            <span className="text-2xl font-bold text-rose-400 tracking-tight">
              Dressmaker <span className="text-white">Wiki</span>
            </span>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm text-gray-300 hover:text-rose-400 transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="px-4 py-3 space-y-2">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-3 py-2 rounded-md text-base text-gray-300 hover:text-rose-400 hover:bg-gray-800 transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
