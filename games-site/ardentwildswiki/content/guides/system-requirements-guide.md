...
- **The minimum GPU (GTX 1060 / RX 580) is the floor, not the target** — for smooth performance on a voxel game, a newer GPU than the minimum is a reasonable expectation (待验证)
...
The game requires OpenGL 4.6 support. OpenGL is CPU-driven compared to newer APIs like Vulkan or DirectX 12, which reinforces the point that **Ardent Wilds leans on the CPU**. On lower-end machines, the CPU — not the GPU — is likely to be the bottleneck in heavily loaded worlds (待验证).

### Storage: 15 GB listed, smaller reported

The official figure is **15 GB available space**. Some third-party listings report a smaller download (~3–5 GB, 待验证). Treat 15 GB as the planning number; the actual install may be smaller.
...
|----------------|-------------|
| **Above minimum (modern mid-range+)** | Comfortable. The demo's starting regions run smoothly on hardware at or above the minimum (community reports, 待验证) |
| **At minimum spec** | Playable with settings tuned down; expect frame dips when the world is heavily modified (craters, large builds) — voxel games degrade with world churn (待验证) |
...
| **Steam Deck** | No official Steam Deck compatibility badge as of August 2026 (待验证). The demo is the free test |

## Settings & FPS Tips

No official settings list exists yet, but the confirmed technical profile (voxel world, OpenGL 4.6, CPU-heavy) makes these the highest-impact levers to try first — in the in-game settings menu when the demo ships them (待验证):

1. **Lower view/render distance first** — in voxel games, draw distance is usually the single biggest FPS lever because it multiplies the amount of world being simulated (待验证)
...
- **Heavily modified terrain costs performance** — each crater and structure is persistent world state (confirmed design), and large persistent state is the classic voxel-game FPS killer (待验证)
...
- **Plan large demolitions during downtime** — a big controlled detonation run is fine, but expect a hitched frame or two right after (待验证)
...
Not listed. The store page lists Windows only as of August 2026. A native macOS or Linux version has not been announced (待验证).
