...
> 本文件位于 monorepo 根目录，所有子项目启动时都会自动加载（Codex 从 cwd 向上遍历）。
> 子目录的 AGENTS.md 只需写项目特有内容，通用约定放这里。
...
3. 在 Codex 内说「切换到 x」即可换目录
4. `cc -l` 列项目，`cc -e` 编辑项目列表

## 项目隔离

每个子目录启动时：
- **记忆独立**: `~/.Codex/projects/<编码路径>/memory/` 各有一份
- **AGENTS.md 叠加**: 本文件（基类）+ 子目录 AGENTS.md（子类），两层合并
- **会话独立**: 不同项目的对话历史互不干扰
